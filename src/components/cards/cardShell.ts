 ...
 * elevation (MASTER.md §49/§82). Used by FeatureCard so the shell itself —
 * padding, radius, shadow, elevation — has one definition; only the hover
 * accent color differs per card variant (kept as a lookup so a future
 * card variant can add its own accent here instead of duplicating the shell).
 */
